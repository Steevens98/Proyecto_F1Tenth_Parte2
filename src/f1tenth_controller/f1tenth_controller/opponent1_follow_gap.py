...
class Opponent1FollowTheGap(Node):
    def __init__(self):
        super().__init__('opponent1_follow_gap')
        
        # Suscripciones (OPONENTE 1)
        ...
        # PARÁMETROS DEL CONTROLADOR (MODERADO)
        self.max_speed = 2.5
        self.min_speed = 0.2
        ...
        self.get_logger().info('=== OPONENTE 1 - CONTROLADOR MODERADO ===')
    ...
    def scan_callback(self, msg):
        ...
        if not gaps:
            ...
            speed = max(1.2, self.prev_speed * 0.9)
            ...
            # 8. VELOCIDAD ADAPTATIVA (TODAS MODERADAS)
            ...
            if front_distance < 0.6:
                ...
                speed = min(1.8, front_distance * 0.8)
            else:
            ...
            speed = min(speed, front_distance * 1.5)
...
def main(args=None):
    ...
    node = Opponent1FollowTheGap()
    
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        node.get_logger().info('Cerrando oponente 1...')
    finally:
        node.stop_robot()
        node.destroy_node()
        rclpy.shutdown()
